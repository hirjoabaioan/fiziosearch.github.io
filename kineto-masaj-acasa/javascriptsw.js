...
// Carousel for prices
if (currentPage === "/templates/abonamente.html") {
  handleWindowResize();
...
if (currentPage === "/templates/terapeuti/terapii.html") {
  ...
  for (let hour = 0; hour < MAX_HOURS; hour++) {
    ...
    optionHourElement.value = hour < 10 ? "0" + hour : hour;
    optionHourElement.text = hour < 10 ? "0" + hour : hour;
  ...
  for (let min = 0; min < MAX_MINUTES; min += MINUTES_INCREMENTATION) {
    ...
    optionMinElement.value = min < 10 ? "0" + min : min;
    optionMinElement.text = min < 10 ? "0" + min : min;
    dropDownMinutesInsert.add(optionMinElement);
  }
}

// Patient Therapies page - Therapist Point Of View
if (currentPage === "/templates/terapeuti/pagina-pacient.html") {
  ...
  function calculateDuration(therapy, startHour) {
    if (patientTherapies.some((element) => element.Terapie === therapy)) {
...
const inputDropdown = $("#dropdownService");
...
const kinetoterapieSpecializations = ["Kinetoterapie", "Cardiologie", "Balnceologie", "Geriartrie", "Limfedem", "Neurologie", "Pediatrie", "Traumatologie", "Recuperare sportivă"];

const masajSpecializations = ["Masaj", "Masaj de relaxare", "Masaj terapeutic", "Masaj anticelulitic", "Masaj de drenaj limfatic", "Reflexoterapie", "Cupping", "Masaj cu miere", "Masaj cu bambus", "Acupunctură"];
...
populateDropdown(servicesDropdown, services);

inputDropdown.on("change", function () {
  const selectedService = inputDropdown.val();
  if (selectedService === "Kinetoterapie") {
    populateDropdown(specializationDropdown, kinetoterapieSpecializations);
  } else if (selectedService === "Masaj") {
    populateDropdown(specializationDropdown, masajSpecializations);
  } else {
    populateDropdown(specializationDropdown, services);
  }
});
...
const collapsibleRows = document.querySelectorAll(".program-table tr.collapsible");

collapsibleRows.forEach((row) => {
  const contentRow = row.nextElementSibling;
  contentRow.style.display = "none";
  row.addEventListener("click", function () {
    collapsibleRows.forEach((otherRow) => {
      if (otherRow !== this) {
...
if (scheduleTable) {
  scheduleTable.querySelectorAll("thead th").forEach((th, index) => {
    const sortButton = document.createElement("button");
    sortButton.classList.add("sort-button");
    sortButton.textContent = "▼";
    sortButton.addEventListener("click", () => {
      const asc = sortButton.textContent === "▲";
      sortTable(scheduleTable, index, asc);
...
const isLogged = document.querySelector("#loggedIn");
